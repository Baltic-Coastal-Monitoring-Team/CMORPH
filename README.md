...
The Data Visualizer notebook is designed to provide a clear and intuitive visual summary of morphological structure across coastal profiles. Each profile is displayed as a vertical column segmented into distinct elevation zones – from waterline through beach and dune to area behind top of dune. This helps identify patterns of erosion, accretion, and profile shape variability across multiple transects.

### What does the program do?

- Loads the `finder.csv` file generated by the **Finder** module for a selected date.
- Extracts characteristic elevation markers for each profile:
- `last_zero` (approximate shoreline/waterline)
- `bottom` (base of the dune or cliff)
- `top` (crest of the dune or cliff)
- Creates a column-style diagram where each profile is represented as a vertical line with colour-coded segments.
- Overlays circular markers to clearly indicate key elevation points.
- Exports the figure as a high-resolution `.jpg` file to a predefined output directory.

### Key Features

- Consistent layout and visual formatting optimised for publication and reporting.
- Colour-coded areas: blue for water, sandy tones for beach and dune, grey for backdune zone.
- Fully customisable spacing and column width via parameters in the script (`bar_width`).
- Outputs figures with profile IDs and markers that facilitate interpretation and comparison.

### Input Requirements

- One `finder.csv` file generated from the Finder module.
- Elevation columns required: `profile_id`, `last_zero` (it could be replaced to first_zero), `bottom`, `top`

### Output

- One `.jpg` figure per input folder.
- File is saved in `data/data-visualiser/` with the name pattern: 
`data-vis_FOLDER-NAME.jpg` (e.g., `data-vis_2022-02.jpg`)

    <p align="center">
      <img src="https://c5studio.pl/cmorph/data-visualizer.jpg" alt="data-visualizer" width="auto">
    </p>
